...
struct Paper: Codable, Identifiable, Equatable {
    ...
    let lastAffectedCommitTime: Date?
    let lastAffectedCommitAuthor: String?
    let createdAt: Date
    let updatedAt: Date
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case lastSyncedAt, lastAffectedCommitTime, lastAffectedCommitAuthor
    ...
    init(from decoder: Decoder) throws {
        ...
        if let lastAffectedMs = try container.decodeIfPresent(Double.self, forKey: .lastAffectedCommitTime) {
            lastAffectedCommitTime = Date(timeIntervalSince1970: lastAffectedMs / 1000)
        } else {
            lastAffectedCommitTime = nil
        }

        lastAffectedCommitAuthor = try container.decodeIfPresent(String.self, forKey: .lastAffectedCommitAuthor)
