          ...
          <div className="flex w-[115px] flex-col items-center justify-center">
            ...
            {repo.syncStatus === "error" && (
              <button
                ...
                disabled={isSyncing}
                className="mt-1 text-xs text-red-600 hover:text-red-700 disabled:opacity-50 dark:text-red-400 dark:hover:text-red-300"
              >
                Retry
              </button>
            )}
            ...
            <span className="mt-0.5 whitespace-nowrap text-[11px] text-gray-400 dark:text-gray-500">
              {lastCheckedLabel}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 lg:hidden">
          ...
          {repo.syncStatus === "error" && (
            <button
              onClick={() => onSync(repo._id)}
              disabled={isSyncing}
              className="inline-flex items-center gap-1 rounded-md border border-red-200 px-2 py-0.5 text-xs text-red-600 hover:bg-red-50 disabled:opacity-50 dark:border-red-800 dark:text-red-300 dark:hover:bg-red-900/30"
            >
              Retry
            </button>
          )}
...
const SyncStatusBadge = memo(function SyncStatusBadge({ repo }: { repo: Repository }) {
  ...
  if (repo.syncStatus === "error") {
    return (
      <span className="inline-flex items-center gap-1 whitespace-nowrap rounded-md bg-danger-50 px-2 py-0.5 text-xs font-medium text-danger-700 dark:bg-danger-500/20 dark:text-danger-400">
        <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
        Sync failed
